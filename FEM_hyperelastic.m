...
%
infile=fopen('hyperelastic_quad4.txt','r');
...
%          Fix constrained nodes.
         for n = 1 : nfix
           ...
           K(rw,rw) = 1.;
           b(rw) = loadfactor*fixnodes(3,n)-w(rw);
         end
%
...
%
    forcevdisp(2,step+1) = loadfactor*dloads(3,1);
    forcevdisp(1,step+1) = w(3);
  end 
...
%
function C = materialstiffness(ndof,ncoord,B,J,materialprops)
...
%
function stress = Kirchhoffstress(ndof,ncoord,B,J,materialprops)
...
%
function rel = elresid(ncoord,ndof,nelnodes,elident,coord,materialprops,displacement)
%
          ...
          for b = 1:nelnodes
            for k = 1:ndof
              ...
              for j = 1:ncoord
                ...
                kel(row,col) = kel(row,col) - ...
                               stress(i,j)*dNdxs(a,k)*dNdxs(b,j)*w(intpt)*dt;
...
%
function resid = globalresidual(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs)
%
