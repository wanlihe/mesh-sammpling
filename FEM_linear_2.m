...
[K,Ke,u_index] = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nfix,fixnodes);
...
nstore = 10;
f_int_sotre = zeros(nnode*ndof,nsteps/nstore);
f_e_int_store = zeros(8,nelem,nsteps/nstore);
...
an = M\(-K*un + F);
an1 = M\(-f_internal(Ke,un,u_index,ndof,nnode,nelem) + F);
...
for n = 1:nsteps
    n
    % new
    [f_int,f_e_int] = f_internal_2(Ke,un +dt*vn + 0.5*(1.-beta2)*dt*dt*an,u_index,ndof,nnode,nelem);
    rn = F - f_int;
    ...
    if (count == nstore)
        f_int_sotre(:,n/nstore) = f_int;
        f_e_int_store(:,:,n/nstore) = f_e_int;
        count = 0;
    end
    count = count + 1;
end
...
X_train = f_e_int_store(:,:,1:5000);
Y_train = f_int_sotre(:,1:5000);
X_test = f_e_int_store(:,:,5001:end);
Y_test = f_int_sotre(:,5001:end);
save data.mat X_train Y_train X_test Y_test

% end

%================= f_internal ==================================
function f_int = f_internal(Ke,u,u_index,ndof,nnode,nelem)

f_int = zeros(ndof*nnode,1);

for lmn = 1:nelem
    f_int(u_index(:,lmn)) = f_int(u_index(:,lmn)) + Ke(:,:,lmn) * u(u_index(:,lmn));
end

end

function [f_int,f_e_int] = f_internal_2(Ke,u,u_index,ndof,nnode,nelem)

f_int = zeros(ndof*nnode,1);
f_e_int = zeros(8,nelem);

for lmn = 1:nelem
    temp = Ke(:,:,lmn) * u(u_index(:,lmn));
    
    f_int(u_index(:,lmn)) = f_int(u_index(:,lmn)) + temp;
    f_e_int(:,lmn) = temp; 
end
...
%
function [Stif,Ke,u_index] = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nfix,fixnodes)
%
%   Assemble the global stiffness matrix
%
...
% new
Ke = zeros(8,8,nelem);
u_index = zeros(8,nelem);

%
%  Fix constrained nodes.
Kc = zeros(ndof*nnode,ndof*nnode);
for n = 1: nfix
    rw = ndof*(fixnodes(1,n)-1) + fixnodes(2,n);
    for cl = 1 : ndof*nnode
        Kc(rw,cl) = 1;
...
for lmn = 1:nelem
    ...
    for a = 1:nelnodes(lmn)
        for i = 1:ndof
            % new
            temp = [temp;ndof*(connect(a,lmn)-1)+i];
            temp1 = [temp1;ndof*(a-1)+i];
